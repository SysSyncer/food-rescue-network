...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const claimedDonations = await VolunteerAssignment.find({ volunteer_id: volunteerId })
    .populate({
      path: "donation_id",
      model: FoodDonation, // ✅ Explicitly mention the model
      select: "title description quantity food_type pickup_address expiry_date status", // ✅ Select specific fields
    });

    return NextResponse.json(claimedDonations);
  } catch (error) {
    console.error("Error fetching claimed donations:", error);
    return NextResponse.json({ error: "Failed to fetch claimed donations" }, { status: 500 });
  }
}
